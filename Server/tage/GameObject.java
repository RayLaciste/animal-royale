...
public class GameObject
{
	...
	*/
	...
	/** Rotates the GameObject around its upward-facing axis by the specified angle. */
	public void yaw(float angle) {
		Matrix4f oldYaw = new Matrix4f(this.getLocalRotation());
		Vector3f up = this.getLocalUpVector();

		Matrix4f rot = new Matrix4f().rotation(angle, up);
		Matrix4f newRot = rot.mul(oldYaw);

		this.setLocalRotation(newRot);
	}

	/** Rotates the GameObject around its right-facing axis by the specified angle. */
	public void pitch(float angle) {
		Matrix4f oldPitch = new Matrix4f(this.getLocalRotation());
		Vector3f right = this.getLocalRightVector();

		Matrix4f rot = new Matrix4f().rotation(angle, right);
		Matrix4f newRot = rot.mul(oldPitch);

		this.setLocalRotation(newRot);
	}

	/** Rotates the GameObject around its forward-facing axis by the specified angle. */
	public void roll(float angle) {
		Matrix4f oldRoll = new Matrix4f(this.getLocalRotation());
		Vector3f fwd = this.getLocalForwardVector();

		Matrix4f rot = new Matrix4f().rotation(angle, fwd);
		Matrix4f newRot = rot.mul(oldRoll);
